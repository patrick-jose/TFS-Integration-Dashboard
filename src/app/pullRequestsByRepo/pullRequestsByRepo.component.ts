...
export class PullRequestsByRepoComponent implements OnInit {
  ...
  ngOnInit() {
    this.repoName = this.data.receiveChosenRepoName();
    
    if (this.repoName != undefined) {
      this.data.getPullRequestsByRepo()
      .subscribe((data) => {
        this.pullRequests = data;
        if (Object.keys(data).length > 0 )
          this.loadChart();
      });
      this.data.getPullRequestByRepoDataChart().subscribe(data => {
        this.chartData = data;
  ...
  loadChart() {
    ...
    this.config = {
      ...
      options: {
        title: {
          text: 'Histórico de Pull Requests Abertos - ' + this.repoName,
          display: true,
          fontSize: 25,
